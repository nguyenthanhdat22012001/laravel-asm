...
<table class="table">
  ...
  @foreach($promotions as $item)
    <tr>
      <td>{{$item->promo_sku}}</td>
      <td>{{number_format($item->promo_price)}} đ</td>
      <td>{{$item->promo_active == 1 ? 'Hiện' : 'Ẩn' }}</td>
      <td>
      <a type="button" href="{{route('get_backend.promotion.edit',$item->id)}}" class="mb-1 btn btn-pill btn-outline-warning"><i class="mdi mdi-grease-pencil"></i></a>
      <a type="button" href="{{route('get_backend.promotion.delete',$item->id)}}" class="mb-1 btn btn-pill btn-outline-danger"><i class="mdi mdi-delete"></i></a>
      </td>
    </tr>
    @endforeach
  </tbody>
