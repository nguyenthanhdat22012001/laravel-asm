...
<table class="table">
  ...
  @foreach($products as $item)
    <tr>
      <td>{{$item->p_sku}}</td>
      <td>{{$item->p_name}}</td>
      <td>{{$item->fk_category->c_name ?? '[N/A]'}}</td>
      <td>{{$item->fk_brand->br_name ?? '[N/A]'}}</td>
      <td> <img src="{{asset('images/product\/')}}{{$item->p_avatar}}" alt="" style="width:50px; height:auto"></td>
      <td>{{$item->p_active == 1 ? 'Hiện' : 'Ẩn' }}</td>
      <td>
      <a type="button" href="{{route('get_backend.product.edit',$item->id)}}" class="mb-1 btn btn-pill btn-outline-warning"><i class="mdi mdi-grease-pencil"></i></a>
      <a type="button" href="{{route('get_backend.product.delete',$item->id)}}" class="mb-1 btn btn-pill btn-outline-danger"><i class="mdi mdi-delete"></i></a>
      </td>
    </tr>
    @endforeach
  </tbody>
