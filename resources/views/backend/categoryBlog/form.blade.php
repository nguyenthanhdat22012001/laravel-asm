<h4>@yield('title_form') {{$categoryBlog->cbl_name ?? ''}}</h4>
<form action="{{$route}}" method="post" enctype="multipart/form-data">
@csrf
<div class="card card-default">
        <div class="card-body">
                <div class="form-group">
                    <label for="exampleFormControlInput1">Danh Mục Blog <span style="color:red">*</span> </label>
                    <input type="text" class="form-control" id="exampleFormControlInput1" name="cbl_name" placeholder="Nhập Danh Mục Blog" value="{{ old('cbl_name', $categoryBlog->cbl_name ?? '') }}">
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="cbl_description">{{ old('cbl_description', $categoryBlog->cbl_description ?? '')}}</textarea>
                </div>  
   ...
   <div class="form-footer pt-4 pt-5 mt-4 border-top clearfix">
            ...
            <div class="float-right d-flex">
                 ...
                 @if(isset($categoryBlog))
                    ...
                    <input type="checkbox" {{ $categoryBlog->cbl_active == 1 ? 'checked' : ''}} name="cbl_active" value="1">
                  @else
               
                    <input type="checkbox" checked name="cbl_active" value="1">
                  @endif
                ...
                @if(isset($categoryBlog))
                    ...
                    <input type="checkbox" {{ $categoryBlog->cbl_hot == 1 ? 'checked' : ''}}  name="cbl_hot" value="1">
                  @else
                
                  <input type="checkbox" checked name="cbl_hot" value="1">
                @endif
                  <div class="control-indicator"></div>
                </label>
            </div>
