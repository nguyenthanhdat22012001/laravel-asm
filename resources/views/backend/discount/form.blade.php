<h4>@yield('title_form') {{$discount->d_number ?? ''}}</h4>
<form action="{{$route}}" method="post" enctype="multipart/form-data">
@csrf
<div class="card card-default">
        <div class="card-body">
                <div class="form-group">
                    <label for="exampleFormControlInput1">Giảm Giá <span style="color:red">*</span> </label>
                    <input type="number" step="any" min=0 class="form-control" id="exampleFormControlInput1" name="d_number" placeholder="Nhập Giảm Giá" value="{{ old('d_number', $discount->d_number ?? '') }}">
   ...
   <div class="form-footer pt-4 pt-5 mt-4 border-top clearfix">
            ...
            <di class="float-right">
                 ...
                 @if(isset($discount))
                    ...
                    <input type="checkbox" {{ $discount->d_active == 1 ? 'checked' : ''}} name="d_active" value="1">
                  @else
               
                    <input type="checkbox" checked name="d_active" value="1">
                  @endif
