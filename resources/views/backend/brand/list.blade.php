...
<table class="table">
  ...
  @foreach($brands as $item)
    <tr>
      <td> <img src="{{asset('images/brand\/')}}{{$item->br_avatar}}" alt="" style="width:50px; height:auto"></td>
      <td>{{$item->br_name}}</td>
      <td>{{$item->br_active == 1 ? 'Hiện' : 'Ẩn' }}</td>
      <td>
      <a type="button" href="{{route('get_backend.brand.edit',$item->id)}}" class="mb-1 btn btn-pill btn-outline-warning"><i class="mdi mdi-grease-pencil"></i></a>
      <a type="button" href="{{route('get_backend.brand.delete',$item->id)}}" class="mb-1 btn btn-pill btn-outline-danger"><i class="mdi mdi-delete"></i></a>
      </td>
    </tr>
    @endforeach
  </tbody>
