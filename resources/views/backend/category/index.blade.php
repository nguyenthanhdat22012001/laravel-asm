...
<table class="table">
  ...
  @foreach($categories as $item)
    <tr>
      ...
      <td> <img src="{{asset('images/icon\/')}}{{$item->c_avatar}}" alt="" style="width:50px; height:auto"></td>
      <td>{{$item->c_active == 1 ? 'Hiện' : 'Ẩn' }}</td>
      <td>
      <a type="button" href="{{route('get_backend.category.edit',$item->id)}}" class="mb-1 btn btn-pill btn-outline-warning"><i class="mdi mdi-grease-pencil"></i></a>
      <a type="button" href="{{route('get_backend.category.delete',$item->id)}}" class="mb-1 btn btn-pill btn-outline-danger"><i class="mdi mdi-delete"></i></a>
      </td>
    </tr>
    @endforeach
  </tbody>
