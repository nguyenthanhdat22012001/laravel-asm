<h4>@yield('title_form') {{$categoryChild->cchild_name ?? ''}}</h4>
<form action="{{$route}}" method="post" enctype="multipart/form-data">
@csrf
<div class="card card-default">
        <div class="card-body">
                <div class="form-group">
                    <label for="exampleFormControlInput1">Danh Mục Con <span style="color:red">*</span> </label>
                    <input type="text" class="form-control" id="exampleFormControlInput1" name="cchild_name" placeholder="Nhập Danh Mục Con" value="{{ old('cchild_name', $categoryChild->cchild_name ?? '') }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="exampleFormControlSelect12" name="cchil_parent">
                    <option value="">Không xác định</option>
                    
                    @foreach($categories as $item)
          
                        <option value="{{$item->id}}" {{old('cchil_parent',$categoryChild->cchil_parent) == $item->id ? 'selected' : ''}} >{{$item->c_name}}</option>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" name="cchild_description">{{ old('cchild_description', $categoryChild->cchild_description ?? '')}}</textarea>
                </div>  
   ...
   <div class="form-footer pt-4 pt-5 mt-4 border-top clearfix">
            ...
            <div class="float-right d-flex">
                 ...
                 @if(isset($categoryChild))
                    ...
                    <input type="checkbox" {{ $categoryChild->cchild_active == 1 ? 'checked' : ''}} name="cchild_active" value="1">
                  @else
               
                    <input type="checkbox" checked name="cchild_active" value="1">
                  @endif
